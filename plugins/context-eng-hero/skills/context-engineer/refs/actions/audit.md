...
- **Done when:** From plugin root (Python **3.14+**, see `python-runtime.md`):
...
- **Done when:** All judgment ids evaluated; static ids in rubric **Static** section skipped (already covered); no score or 12-check cap.
...
- **Done when:** Report includes narrative findings (pattern labels), recommended next step; any FAIL recommends `/context-engineer-rewrite` for that path.

## Stop

**Diagnosis only.** No edits unless the user separately runs **Action: rewrite** (or explicit out-of-band ask).
