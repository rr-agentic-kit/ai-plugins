...
| --improve | Parallel compliance + opportunity audits → merge → absorb `fix` then `redesign` under write gates | Running either diagnosis alone; ambient “make it better” without a path | Parent-only executors `refs/executors/compliance.md` + `opportunity.md` via `generalPurpose` Task + Caller Load; action `refs/actions/improve.md` |
