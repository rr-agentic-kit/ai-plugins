...
| refactor (builder) | Task-step / handoff: rr-refactor on MR ∩ build-touched; after build, before review | Casual cleanup; Sonar-only | Orchestrate skips when intersection empty |
| review (builder) | Task-step / handoff review: rr-review; orchestrate review forces `--fix --all --endless` | Casual code glance; Challenge (Discover) | Explicit `--review` may be report-only; `--endless` loops until clear or max-epochs |
