...
| Challenge (review) | rr-review gate that re-triages assess rows before report/fix/ci; consumers use **keep** only | Discover `--challenge` / challenge-layers | Persist `{assess-stem}-challenge.md` under `.ai/review/<runId>/` (scratch); unchallenged challengeable rows stop the report |
...
| orchestrate | Builder advances slice pipeline (prepare→…→optional ship→delivered) under drive×scope | Handoff (one nested skill then stop); **feature** (mint + task-only) | Dual-mode invariant with handoff; default `auto` × `step` |
...
| task-step | Ordered implementer step inside a prepare task’s **Steps** | Full capability atom / prepare task; **isolated step run** (the outer spawn loop) | Pipeline: plan→build→refactor→review→validate⇄ship (shippable) per step |
| isolated step run | `auto` × `task`\|`slice` orchestrate cell: one sequential `generalPurpose` Task per remaining task-step; parent owns prepare/gate/ship/validate | Parent-inline stages (`--step`, `--manual`, `--feature`); parallel worktrees | Context isolation only — same tree; [slice-pipeline.md](skills/rr-builder/refs/slice-pipeline.md) |
...
| refactor (builder) | Task-step / handoff: rr-refactor on MR ∩ build-touched; after build, before review | Casual cleanup; Sonar-only | Orchestrate durable: lean `{NNNN}-{step}.refactor.md`; `.ai/refactor/` = scratch (no required `report.md`); skips when intersection empty |
| review (builder) | Task-step / handoff review: rr-review; orchestrate review forces `--fix --all --endless` | Casual code glance; Challenge (Discover) | Orchestrate terminal: `{NNNN}-{step}.review.md`; scratch `.ai/review/`; handoff without cursor keeps `REVIEW_DIR/report.md` |
| endless review | rr-review loop: assess→Challenge→fix→re-assess until code+test zero-keep (or security-only `warnings`) or `--max-epochs` | rr-test-endless coverage loop | Default max-epochs 5; [endless.md](skills/rr-builder/rr-review/refs/endless.md) |
| step-validate | Per-step Goal/Verify (+ Forge when shippable) assessment; marks plan Verify checkboxes | Full task-validate; CI job | Report `{NNNN}-{step}.validate.md`; [task-validate.md](skills/rr-builder/refs/task-validate.md) |
| task-validate | After all steps: task Verify/Goal/Obligations assessment | Slice validate; step-validate | Report `{NNNN}.task-validate.md`; flips task Verify `- [x]` on item PASS |
