...
| orchestrate | Builder advances slice pipeline (prepare→…→optional ship→delivered) under drive×scope | Handoff (one nested skill then stop) | Dual-mode invariant with handoff; default `auto` × `step` |
