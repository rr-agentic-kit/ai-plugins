...
- All available planning docs (exec-summary through frd) plus `items.json` if present
...
1. Read `payload.static_validation`. If `skipped`, you may flag broken parents and `build != none` on non-ready items. If the script ran, **do not** re-check refs, numbering, kind, required keys, spec/build legality, or md/json drift.
2. Load all planning docs from input or output-dir.
3. Scan each doc against full blind-spot taxonomy (judgment):
