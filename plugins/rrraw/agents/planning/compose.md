...
8. Fill the **level’s** priority method on leaves (MoSCoW / Kano / triad) using the posture legend for PRD/ES MoSCoW. Unranked leaves: native key `—`. Containers: unmarked. Exec-summary must include the unranked Posture, What-must-be-true, and Viability-verdict leaves. Ranked leaves: write `_rationale_: r-NNN` from `level_facts`. If a ranked leaf has no rationale id → `clarifications_needed` (`severity: blocking`); do not invent an `r-*`. Unranked leaves omit `_rationale_` unless `level_facts` supplies one.
