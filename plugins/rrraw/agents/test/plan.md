...
2. **Trim-before-add:** All `maintain` / `trim` / `redundant` work must complete before any `add` step runs in the same epoch:
   ...
   - Set `maintain_before_add: true` in complete-missing chains.
   - `add` track empty until maintain queue cleared or `wontfix` recorded in `constraints_applied`.
3. Split work into `maintain` (strengthen or trim existing) and `add` (new tests) tracks.
...
7. Set `steps_total` = `maintain.length + add.length`.
8. Record `constraints_applied` (epoch budget, target, pyramid stance, trim-before-add when triggered).
...
Required fields: `maintain`, `add`, `constraints_applied`, `steps_total`, `maintain_before_add`.
