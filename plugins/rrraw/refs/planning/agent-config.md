...
**Owner:** Injection version, the one load line on root agent SoT files, and the `agent.plan.md` body. Skill is the only writer of `docs/rr/agent.plan.md` and of that one line on root SoT files.
...
injection:
  version: 4
  load_line: "Read `docs/rr/agent.plan.md` before any planning, pairing, or version work. Do not remove this line."
```
...
`<phase-dir>` is `docs/rr/{track}/discovery/` or `docs/rr/{track}/plan/` (sync resolves `docs/` then writes parking under `docs/rr/`). First compose uses the same command after writing phase status. `--setup` runs the same writes as part of its section list ([setup.md](setup.md)).

## agent.plan.md template

Body source is sibling [`agent.plan.md`](agent.plan.md). Exact bytes written to `{PROJECT_ROOT}/docs/rr/agent.plan.md`. Overwrite the project file when `injection.version` advances. Skill-owned. Not validator cascade input.
