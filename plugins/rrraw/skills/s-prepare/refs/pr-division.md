...
**Audience:** After all L2 tasks are `detailed`. No forge open (**s-ci** later).
...
Prepare `complete` after PR map written. Hand off to humans / later orchestrate (**plan** owns branch + `ship_after`) + **s-ci** on **ship**.
