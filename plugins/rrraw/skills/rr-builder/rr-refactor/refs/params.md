...
**Audience:** **rr-refactor** via **rr-builder** handoff or orchestrate refactor stage. Parse flags into this block **before** `Read`ing the refactor orchestrator skill. Unknown flags → **stop** with one-line error.
...
**MR recipe:** `git merge-base HEAD '@{upstream}'`; on failure `origin/main` or `origin/master`; `git diff --name-only "$MERGE_BASE"..HEAD`. Positional paths narrow further. Same recipe as **rr-review** report/fix scope.
