...
**Audience:** **rr-review** when `endless: true` — always when `outcome: fix` (orchestrate review **and** handoff `--fix`; `--endless` alone still requires / forces fix). Shape mirrors rr-test-endless exit caps without coverage heuristics.
