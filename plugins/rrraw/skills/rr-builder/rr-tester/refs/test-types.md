...
Prefer more unit than integration than E2E. Rough shape:

| Layer | Share of suite | Role |
|-------|----------------|------|
| Unit | 70-80% | Business logic, validation, composables |
| Component | Frontend | Render + interaction in simulated DOM |
| Integration | 20-30% | Boundaries, API clients, routing |
...
Contract tests apply when multi-service architecture has external consumers, cross-team boundaries, or independent deployment schedules. Prefer consumer-driven checks at the published API boundary; keep full integration coverage for end-to-end behavior.
...
└─ NO → Does it have external dependencies (DB, HTTP, filesystem, real cluster)?
    ...
    │   ├─ YES → UNIT TEST
    │   └─ NO → Refactor production per `rr-coder/refs/testability.md` (**extract-pure-core**), then UNIT TEST; if refactor is out of scope → **BLOCKED (code lane)** — do not mock around a god method
    │
