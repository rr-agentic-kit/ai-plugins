...
Lane → path mapping SoT: [refs/routing.md](refs/routing.md) **Handoff load table** (carries each lane's stop notes). Seven specialist skills — s-prepare, s-coder, s-tester, s-security, s-review, s-refactor, s-test-endless — are listed in `plugin.json` for user `@s-*` invoke. **rr-builder** path-loads them via explicit `Read` during orchestrate/handoff; agents must not auto-select.

**Path pattern:** `skills/s-<lane>/SKILL.md` (e.g. `skills/s-coder/SKILL.md`).

All `s-*` skills set `disable-model-invocation: true` (manual user invoke or explicit parent `Read` only).
...
| [refs/ship.md](refs/ship.md) | Orchestrate **ship** stage (branch/base resolve → s-ci) |
| [refs/task-validate.md](refs/task-validate.md) | Task / step validate stages |
| [refs/pr-validate.md](refs/pr-validate.md) | Post forge-landing **pr-validate** (CI wait/fix via s-ci) |
