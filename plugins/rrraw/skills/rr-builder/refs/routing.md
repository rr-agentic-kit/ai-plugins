...
**Path pattern:** every `Load` below is a top-level specialist skill — `skills/s-<lane>/SKILL.md`.

| `payload.lane` | Load | Notes |
|----------------|------|-------|
| `prepare` | `skills/s-prepare/SKILL.md` | Stop after L3; no auto-chain to coder |
| `coder` | `skills/s-coder/SKILL.md` | Implement/refactor only for scoped request |
| `tester` | `skills/s-tester/SKILL.md` | Forward test payload; parent does not re-parse conflicts |
| `security` | `skills/s-security/SKILL.md` | Report-only |
| `review` | `skills/s-review/SKILL.md` | Nested `--code|--test|--security|--all|--fix|--ci|--endless` per `skills/s-review/refs/params.md` |
| `refactor` | `skills/s-refactor/SKILL.md` | Fixed-point behavior-invariant refactor; `Task` `refactor-collector` only when >50 files per `skills/s-refactor/refs/agent-index.md` |
| `add_endless_test` | `skills/s-test-endless/SKILL.md` | Coverage-first multi-epoch loop; `Task` for gateway + leaf agents per `skills/s-test-endless/refs/orchestration.md` |

After review `--ci` findings: **Read** `skills/s-ci/SKILL.md` for forge POST. Do not open PR from other lanes.

Local worktree / destructive git during review `--fix`: **Read** `skills/s-git/refs/` as s-review already directs — does not replace standalone **s-git**.
...
| Stage | Pointer |
|-------|---------|
| **prepare** | Full `skills/s-prepare/SKILL.md` |
| **plan** | [plan-knowledge.md](plan-knowledge.md) + [plan-schema.md](plan-schema.md) |
| **build** | Full `skills/s-coder/SKILL.md` **and** `skills/s-tester/SKILL.md` |
| **refactor** | Full `skills/s-refactor/SKILL.md` — scope = MR ∩ **build-touched** paths; see [slice-pipeline.md](slice-pipeline.md) |
| **review** | Full `skills/s-review/SKILL.md` with forced `--fix --all --endless` |
| **step_validate** / **task_validate** | [task-validate.md](task-validate.md) |
| **ship** | [ship.md](ship.md) then `skills/s-ci/SKILL.md` |
| **pr_validate** | [pr-validate.md](pr-validate.md) then `skills/s-ci/SKILL.md` |
| **slice_validate** | [slice-validate.md](slice-validate.md) |
| **delivered** | Stop — point to **s-ci** for residual unshipped work |

Run loop (auto/manual × next/step/task/slice, readiness, cursor): [slice-pipeline.md](slice-pipeline.md).

### Review lane delegation (inside s-review)

When `s-review` runs assess:

| Lane | Nested skill |
|------|--------------|
| `code` | `skills/s-coder/SKILL.md` |
| `test` | `skills/s-tester/SKILL.md` |
| `security` | `skills/s-security/SKILL.md` |

Review orchestration (brief, chunk, Challenge, merge report) stays in **s-review**; lane rubrics stay in nested skills.
...
- Dumping s-coder Required Knowledge / language matrices on **plan** (use [plan-knowledge.md](plan-knowledge.md) only).
...
- Opening PR/MR inside builder without **s-ci** — boundary SoT: [anti-overlap.md](anti-overlap.md) s-ci section.
