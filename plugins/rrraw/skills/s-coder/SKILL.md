---
name: s-coder
description: /s-coder — production code standards (SOLID, CPNNN, architecture) for scoped implement/refactor.
...
Apply production code standards (SOLID, CPNNN rubric, architecture, observability, language refs) to **application source** only. Tests and test strategy live under **s-tester**; s-tester loads this skill so test **code** matches implementation rules.

## When to use

- Implement or refactor production/application source via **rr-builder** `--coder` (full-skill handoff) or **s-review** code lane
...
| Need | Use instead |
|------|-------------|
| Test gaps, MISSING verdicts | **s-tester** |
| OWASP / exploitability | **s-security** |
| MR inline POST | **s-ci** after **s-review** `--ci` |
...
Fix path: inline apply in the parent session. When **s-review** `--fix` routes code findings, follow the review plan steps.
