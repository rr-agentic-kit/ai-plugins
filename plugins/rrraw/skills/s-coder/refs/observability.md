...
Map findings to **CP027** / **CP028** / **CP029** in [`compliance-rubric.md`](compliance-rubric.md). Secrets/tokens/PAN/raw PII in log args → **CWE-532**.
...
| **Safety** | Redact secrets, tokens, PAN, raw PII; fail closed (omit field or use redactor output) | Passwords, bearer tokens, card data, unmasked PII; log raw then “hope” downstream scrubbing → **CWE-532** | Aggregators retain raw forever |
...
**Not worth a log** (flag as noise / Suggestion–Warning under **CP027** when present): happy-path chatter every request, restating what metrics already cover, dumping full request/response bodies, logging inside tight loops, duplicate identical messages at multiple layers for the same event at the same level.
...
**Not worth a metric** (flag unnecessary / vanity under **CP028** when peers already have a clean set — or Suggestion when introducing noise): per-method “entered”, counters that duplicate HTTP server defaults without extra labels of value, high-cardinality dimensions, one-off debug counters left in production, metrics with no conceivable alert or dashboard consumer.
...
**Missing vs excess:** **CP028** flags **missing** meters when peers instrument **and** **unnecessary** meters (vanity / duplicate / high-noise). Escalate schema **removal**; do not silent-delete series in refactor without human (breaking dashboards).

## When not to flag

- Pure domain / library modules with **no** runtime edge (no HTTP, consumer, job, or CLI I/O) — do not demand metrics culture (**CP028** demote).
...
- No MDC/OTEL/tracing already present — do **not** invent propagation (**CP029** do-not-flag for missing correlation).
- **No peer redactor culture** — omit the PII field; do **not** invent a redact library or facade (**CP027** do-not-flag for “missing redactor”).
- Framework auto-metrics only (HTTP server defaults) with no extra vanity labels — demote under **CP028**; do not demand removal of framework defaults.
- Dashboard, alert rule, sampling policy, or Helm sidecar gaps — out of coding-lane scope; operate with host observability tooling when telemetry exists.
...
- CP007 already covers empty catch / log-and-ignore — do not restate as CP027 unless the **existing** log call itself is unstructured, wrong-level, or noisy; secrets/PII in args → **CWE-532**.
...
Disposition values: `fix` \| `clarify` \| `escalate_human`. Cite **CP027**–**CP029** or **CWE-532** when applicable; type **`observability`** on Phase 8 rows for these findings (language Prefer/Avoid that are not observability stay `language_ref`).
