...
2. **Optional dry-run** — `s-ci pull-dependabot --dry-run` once if the user wants a branch list. Paste ≤50 lines of JSON. Done.
3. **Run (one shell)** — `s-ci pull-dependabot --verify-cmd '<cmd>'` (e.g. `just test`). Do **not** chat-orchestrate per-branch merge/verify/delete. Progress is stderr; parse stdout JSON only. Done: envelope in context.
...
If `s-ci pull-dependabot` returns invalid choice / unknown command:
