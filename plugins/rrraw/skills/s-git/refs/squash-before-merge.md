...
**When:** Local squash of the current branch onto `origin/main` or `origin/master` (pre-merge on forges without native squash-on-merge, or any repo that wants one commit before merge). GitLab/GitHub squash-on-merge at the forge does **not** need this script. Force-push here is still **pre-forge** squash (s-git), not PR/MR create.

**Script:** `skills/s-git/scripts/squash-onto-base.sh` — invoke from [scripts/README.md](../scripts/README.md). Confirm with the user before force-push.
