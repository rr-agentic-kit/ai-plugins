...
└─ NO → Does it have external dependencies (DB, HTTP, filesystem, real cluster)?
    ...
    │   ├─ YES → UNIT TEST
    │   └─ NO → Refactor production per `s-coder/refs/testability.md` (**extract-pure-core**), then UNIT TEST; if refactor is out of scope → **BLOCKED (code lane)** — do not mock around a god method
    │
