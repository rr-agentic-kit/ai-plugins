---
name: s-refactor
description: /s-refactor — fixed-point behavior-invariant coder-rule refactoring for a scoped path set.
...
| **`skills/s-coder/SKILL.md`** + **`refs/code.principles.md`** | Before assess |
| Stack detect → language refs + **`skills/s-coder/refs/observability.md`** | Before assess on scoped stack |
