...
Parses lanes, mints `runId`, builds brief, chunks large scope, assesses via nested skills, runs Challenge, then branches to report / inline fix / endless fix loop / **s-ci** handoff.

**Out of scope:** standalone PR open or pipeline-only debug (**s-ci** without review); lane rubrics themselves (owned by nested skills).
...
- Implement-only or test-only without review flags → route to **s-coder** / **s-tester** via parent
...
- **Forge handoff** — `--ci` loads **s-ci** after Challenge; review does not embed glab/gh scripts
...
`--fix` → endless loop per `refs/endless.md` (fix-routing inline each epoch); `--ci` → **s-ci** after Challenge; refuse POST on unchallenged blocker-tier rows.
...
- `disable-model-invocation: true` — manual `@s-*` / slash invoke
