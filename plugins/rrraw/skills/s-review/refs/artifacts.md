...
**Purpose:** Sole path + filename + body-template index for **s-review** runs. Nested skills and severity refs **point here**; do not invent a second path table.
...
**Mint helper (stdout → value only)** — same recipe style as `s-refactor/refs/artifacts.md`; do not list directories into context for the LLM to max-scan:
...
| CI preflight | `scope-preflight.json` | **s-ci** envelope — point only; schema owned by s-ci |
| Brief | `brief.md` | [brief-output.md](brief-output.md) |
| Code assess | `code-assess.md` | Skeleton below — CP emit + [`architecture.md`](../../s-coder/refs/architecture.md) `## Architecture` |
| Test assess | `test-assess.md` | [`s-tester/refs/report-template.md`](../../s-tester/refs/report-template.md) |
| Security | `security-audit.md` | [`s-security/SKILL.md`](../../s-security/SKILL.md) §Output format |
...
CP finding table first, then mandatory architecture section (full AR rules: [`architecture.md`](../../s-coder/refs/architecture.md)):
