...
**Audience:** **s-review** via **rr-builder**. Parse flags into this block before running review steps.
...
**`--ci` scope:** Only `--scope MR` or `--scope PR`. Full-repo `--ci` → stop: `ci requires MR scope`. Use **s-ci** preflight for diff refs and `READ_REF`.
