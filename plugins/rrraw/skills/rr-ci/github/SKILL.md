...
**Fallback (no matching row):** Use the named `rr-ci` subcommand from parent `SCRIPTS-SPEC.md` if listed; else stop — do not invent `gh` flags.

Done: matching row applied (ref loaded or CLI run). Stop: hard failure (`ok: false`, auth missing, no PR).

4. **Default PR create** (after preflight `ready_create`): `gh pr create --draft --fill` unless the user asked otherwise. Title/description from parent templates. Prefer `--squash` merge method when the repo default is squash. Done: PR created or user declined. Stop: preflight not `ready_create`, or create fails.
