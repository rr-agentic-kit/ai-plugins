...
| `sonar-list-issues` | forge-agnostic; default scope = open PR/MR for current branch; `result.project`, `result.total`, `result.issues[]` (`key`, `rule`, `severity`, `type`, `file`, `line`, `message`, `status`); `--lean` → `result.by_file` map path → `{key,rule,severity,line}[]` (omit full `issues[]`/messages); optional `pull_request` / `branch`; flags `-p/--project`, `--pull-request`, `--branch`, `--statuses`, `--lean`; errors `no_open_pr`, `forge_unknown`, `sonar_not_found` |
