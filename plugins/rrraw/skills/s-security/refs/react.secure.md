...
React/frontend-specific secure implementation patterns. CWE Rule IDs live in `secure.owasp.md`.
...
### dangerouslySetInnerHTML with unsanitized user content (XSS — **CWE-79**)
```tsx
// BUG
...
### Storing JWT in localStorage (accessible to XSS — **CWE-79**)
```tsx
...
### Leaking secrets via env vars (bundled, public — **CWE-798**)
```tsx
...
### href={userInput} without protocol validation (javascript: XSS — **CWE-79**)
```tsx
// BUG
...
const allowed = new RegExp("^https://(cdn\\.example\\.com|trusted\\.com)/").test(userAvatarUrl);
