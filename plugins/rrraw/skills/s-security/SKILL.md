---
name: s-security
description: /s-security — OWASP audit with confidence gating; report-only (no auto-fix).
...
Audit scoped code for OWASP Top 10, secrets exposure, and language-specific vulnerabilities. Emit findings only when [refs/confidence.md](refs/confidence.md) gates pass. Report-only — no auto-fix.

## When to use

- **rr-builder** `--security` (full-skill handoff, without `--review`)
- Security lane under **s-review** (also under builder `--auto` **review** via `--fix --all`)
...
| Need | Use instead |
|------|-------------|
| CPNNN / SRP / observability on production code | **s-coder** |
| Test verdicts and coverage | **s-tester** |
| Auto-fix security findings | Not supported |
...
3. **emit** — When loaded by **s-review**, apply [severity-triage.md](refs/severity-triage.md) + confidence gates. Format report per **Output format** below. Done: report-only (even with `--fix`).
...
| Location | Issue | CWE | Confidence | Fix |

### High Issues (should fix)
| Location | Issue | CWE | Confidence |
