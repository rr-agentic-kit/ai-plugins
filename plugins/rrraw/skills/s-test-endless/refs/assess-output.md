...
- Confront evidence against **`skills/s-test-endless/refs/test-heuristics.md`** only. **Verdict precedence:** **`Verdict decision matrix`** in `test-heuristics.md`.
...
**`recipe-id`** values: `extract-pure-core`, `introduce-port`, `inject-time`, `thin-boundary` — per [`testability.md`](../../s-coder/refs/testability.md). Repeat per blocked path when causes differ. Terminal report must list these under **Remaining gaps** / **Blocked (code lane)** per cross-lane escalation section in s-coder testability ref. Do **not** claim coverage or exit rules met for blocked items.
...
4. **Exit conditions** — see **`skills/s-test-endless/refs/exit-conditions.md`**.
