...
**Audience:** **`s-test-endless`** orchestrator (depth 0). **Load first** before lane-specific loop sections.
...
| **Work-pack** | **One** worker **`Task`** = **one** planner-written **`REVIEW_DIR/plans/trp-*.md`** file (default). Packs per **`skills/s-test-endless/refs/work-pack-sizing.md`**. |
...
All **`Task`** prompts must include **`REPO_ROOT`**, **`REVIEW_ID`**, **`REVIEW_DIR`**, **`Scope:`** / chunk scope as applicable, optional **`BRIEF_PATH`** on assess/re-assess, and **`PLUGIN_ROOT=`** so subagents can **Read** plugin paths. See **`skills/s-test-endless/refs/leaf-contract.md`**.
...
2. **Project detection:** Per **`skills/s-test-endless/refs/project-detection.md`**. If **no test framework** → hard stop.
3. **Chunk list:** Build **`chunk_id`** list per **`skills/s-review/refs/chunking.md`** for this scope. Same repo snapshot → same chunks (deterministic). **Skip when `Start: execute`** and checkpoint provides **`assessment_paths_by_chunk`** (§4 queues from **`Glob`**; no assess runs) — do not **Read** `chunking.md` for that run.
...
3. **Optional sanity check:** If multiple paths were announced but any file is missing **`pack_sequence`** / **`sequential_after`** in YAML where required per **`skills/s-test-endless/refs/work-pack-sizing.md`**, **stop** and **Report** — do not invent packing in the runner.
...
1. `skills/s-test-endless/refs/orchestration-core.md` (this file)
2. `skills/s-test-endless/refs/orchestration.md`
3. `skills/s-review/refs/chunking.md` (skip Read when `Start: execute` — Pre-flight §3)
4. `skills/s-test-endless/refs/project-detection.md`
5. `skills/s-test-endless/refs/artifacts.md`
6. `skills/s-test-endless/refs/exit-conditions.md`
7. `skills/s-test-endless/refs/terminal-report.md`
...
| **execute** | `refs/phases/execute.md`, `refs/prompts.md`, `refs/fix-worktree.md`, `skills/s-git/refs/worktree-lifecycle.md` | §4–§6 |
