...
Executors receive **`plan_path`**, **`branch_name`**, **`REPO_ROOT`**, **`PLUGIN_ROOT`**, **`REVIEW_ID`**, **`REVIEW_DIR`**, and **`<base>`** (integration branch — **never** `master`/`main`) — see **`skills/s-test-endless/refs/artifacts.md`** and **`orchestration-core.md`** Pre-flight §1.

**Execute batching:** One plan file = one work-pack = one worker **`Task`**. The **planner** packs per **`skills/s-test-endless/refs/work-pack-sizing.md`**; the runner dispatches from YAML with **`--max-parallel`** cap.
...
| Field | Value |
|--------|--------|
| `<base>` | **Integration branch** (merge target): **local** branch name from pre-flight **`git branch --show-current`** per **`skills/s-test-endless/refs/orchestration.md`** / **`orchestration.md`** Pre-flight §1 — **never** `master` or `main` (ASCII case-insensitive). Worker **refuses** trunk; orchestrator **hard-stops** if still on trunk. |
| `<branch_name>` | YAML frontmatter **`branch:`** on the plan file. **One** git branch + worktree per worker **`Task`**. See **`skills/s-test-endless/refs/orchestration.md`** §4. |
...
Produce one or more markdown plans under REVIEW_DIR/plans/ (mkdir -p; Write each file; packing and YAML per skills/s-test-endless/refs/work-pack-sizing.md).
