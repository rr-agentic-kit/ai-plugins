...
**Load path:** `PLUGIN_ROOT/skills/s-test-endless/refs/toolchain-gateway-prompt.md`

**Use:** **`s-test-endless`** orchestration **§7 verify** — **`Task`** **`test-endless-toolchain`** with **`build`** + **`test`** only. **`Read`** this file, substitute **`REPO_ROOT`** in the block below, and pass the result as the **`prompt`** argument with **`subagent_type`:** **`test-endless-toolchain`**, **`description`:** **`Toolchain gateway: build + test`**.
...
Phases: build, test only (no lint in this gate). Resolve commands per `skills/s-test-endless/refs/project-detection.md`.
