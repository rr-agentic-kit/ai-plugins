#!/bin/sh
# POSIX launcher. Resolves CPython 3.14+ with tiktoken, then execs context_budget_script.
HERE=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
export PYTHONPATH="$HERE${PYTHONPATH:+:$PYTHONPATH}"
PKG="$HERE/context_budget_script"
...
_probe_tiktoken() {
  _py=$1
  PYTHONPATH="$HERE" "$_py" -c "import tiktoken, context_budget_script" >/dev/null 2>&1
  return $?
}

_probe_ok() {
  PYTHONPATH="$HERE" "$@" -c "import sys, tiktoken, context_budget_script; raise SystemExit(0 if sys.version_info >= (3, 14) else 1)" >/dev/null 2>&1
  return $?
}
...
while IFS= read -r _py; do
  ...
  _have_314=1
  if _probe_tiktoken "$_py"; then
    _ready=$_py
    break
  fi
  if [ -z "$_no_tik_py" ]; then
    _no_tik_py=$_py
  fi
done <<EOF
${_list}
...
if [ -n "$_ready" ]; then
  exec "$_ready" -m context_budget_script "$@"
fi

if [ "$_have_uv" -eq 1 ]; then
  if [ -n "$_root" ] && _probe_ok uv run --project "$_root" python; then
    exec uv run --project "$_root" python -m context_budget_script "$@"
  fi
  if [ -z "$_root" ] && [ -f "$PKG/pyproject.toml" ] && _probe_ok uv run --project "$PKG" python; then
    exec uv run --project "$PKG" python -m context_budget_script "$@"
  fi
  if _probe_ok uv run --no-project --python 3.14 --with 'tiktoken>=0.12' python; then
    exec uv run --no-project --python 3.14 --with 'tiktoken>=0.12' python -m context_budget_script "$@"
  fi
fi

if [ -z "$_old_py" ] && [ "$_have_314" -eq 0 ] && [ -z "$_no_tik_py" ]; then
...
if [ -n "$_no_tik_py" ]; then
  printf '%s\n' "context_budget: install tiktoken (uv sync --all-groups from monorepo root)" >&2
  exit 127
fi
printf '%s\n' "no Python interpreter found (tried python3.14, python3, python, uv)" >&2
exit 127
